...
class seasons:
	...
	def seasonDirectory(self, items):
		...
		response = get(control.apollo_link+'?action=meta&type=tv&id=%s&season=1' % (int(items[0]['tvdb'])))
		data = json.loads(response.text)

		for i in items:
			try:
				if not data['seasons'][i['season']]>0:
					continue			
...
class episodes:
	...
	def episodeDirectory(self, items):
		...
		response = get(control.apollo_link+'?action=meta&type=tv&id=%s&season=%s' % (int(items[0]['tvdb']),int(items[0]['season'])))
		data = json.loads(response.text)
		...
		for i in items:
			try:
				if not data['episodes'][i['episode']]==1:
					continue
				...
				url = '%s?action=apollo&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&premiered=%s&meta=%s&t=%s' % (sysaddon, systitle, year, imdb, tvdb, season, episode, systvshowtitle, syspremiered, sysmeta, self.systime)
