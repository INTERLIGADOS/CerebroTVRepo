...
class onethousand(Scraper):
    ...
    def scrape_movie(self, title, year, imdb, debrid=False):
        try:
            ...
            for date,ID,part_url in thumb:
                ...
                if not (title.lower() in link_title.lower() and year in date):
                    continue
    ...
    def get_source(self,movie_link):
        try:
            ...
            match = re.compile('\{"file":"(.+?)","label":"(.+?)"',re.DOTALL).findall(final)
            for link,qua in match:
                if 'subtitle' not in link:
                    link = link.replace('\\','')
                    self.sources.append({'source': 'Googlevideo','quality': qua,'scraper': self.name,'url': link,'direct': True})
        except:
            pass
