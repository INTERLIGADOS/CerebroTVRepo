...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.kratos.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.kratos.artwork').getAddonInfo('path'), 'resources', 'media', theme)
