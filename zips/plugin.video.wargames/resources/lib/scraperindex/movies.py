...
from resources.lib.scrapermods import metacache
    ...
    def movieDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                url = '%s?action=playscraper&title=%s&year=%s&imdb=%s&meta=%s&t=%s' % (sysaddon, systitle, year, imdb, sysmeta, self.systime)
                sysurl = urllib.quote_plus(url)

                path = '%s?action=playscraper&title=%s&year=%s&imdb=%s' % (sysaddon, systitle, year, imdb)
