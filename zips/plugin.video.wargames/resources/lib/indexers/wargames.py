...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://raw.githubusercontent.com/kisakuldlanor/suomynona/master/zipss/001-leviticus-colt-main.xml'
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.wargames_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.wargames_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.wargames_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['Wargames TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def wargames_list(self, url, result=None):
        try:
