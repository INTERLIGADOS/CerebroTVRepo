...
class cFav:
    ...
    def getFavourites(self):
        ...
        sTitle = '[COLOR khaki]You have %s bookmarks[/COLOR]' % (len(row))
        ...
        if compt[0] > 0:
            ...
            oGui.addDir(SITE_IDENTIFIER, 'getFav', '[COLOR red]Error /! \ Link was deleted !!! (' + str(compt[0]) + ')[/COLOR]', 'mark.png', oOutputParameterHandler)
    ...
    def setFavorite(self):
        ...
        if int(oInputParameterHandler.getValue('sCat')) < 1:
            cConfig().showInfo('Error','Getting Favorites not possible at this link')
            return
