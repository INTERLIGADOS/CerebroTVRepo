...
class cGui():
    ...
    def addTV(self, sId, sFunction, sLabel, sIcon, sThumbnail, sDesc, oOutputParameterHandler = ''):
        ...
        oGuiElement.setTitle(sLabel)
        oGuiElement.setIcon(sIcon)
        oGuiElement.setThumbnail(sThumbnail.replace('www.ulantv.com','www.izletv.us'))
    ...
    def createContexMenuWatch(self, oGuiElement, oOutputParameterHandler= ''):
        self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cGui',oGuiElement.getSiteName(),'setWatched','[COLOR azure]Mark Seen / Unseen[/COLOR]')
    ...
    def createContexMenuPageSelect(self, oGuiElement, oOutputParameterHandler):
        ...
        oContext.setTitle('[COLOR azure]Back Site[/COLOR]')
        oOutputParameterHandler.addParameter('sId', oGuiElement.getSiteName())
        oContext.setOutputParameterHandler(oOutputParameterHandler)
        oGuiElement.addContextItem(oContext)
    ...
    def createContexMenuDownload(self, oGuiElement, oOutputParameterHandler= '', status = '0'):
        ...
        if status == '0':
            self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cDownload','cDownload','StartDownloadOneFile','Start the download')
        
        if status == '0' or status == '2':
            self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cDownload','cDownload','delDownload','Remove from list')
            self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cDownload','cDownload','DelFile','[COLOR=red]Remove from list[/COLOR]')
            
        if status == '1':
            self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cDownload','cDownload','StopDownloadList','Stop the download')
    ...
    def createContexMenuba(self, oGuiElement, oOutputParameterHandler= ''):
        ...
        self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cGui',oGuiElement.getSiteName(),'viewBA','[COLOR azure]Trailer[/COLOR]')        
    ...
    def createContexMenuSimil(self, oGuiElement, oOutputParameterHandler= ''):
        ...
        self.CreateSimpleMenu(oGuiElement,oOutputParameterHandler,'cGui',oGuiElement.getSiteName(),'viewsimil','[COLOR azure]Search similar[/COLOR]')
    ...
    def pinKeyBoard(self):
        keyboard = xbmc.Keyboard( '', "ADULT PIN->http://www.ozbeceriksizler.com/forumdisplay.php?66-IPTV-Online-TV", False )
        keyboard.doModal()
        if ( keyboard.isConfirmed() ):
    ...
    def showNumBoard(self, sDefaultNum=''):
        ...
        numboard = dialog.numeric(0, 'Enter the page', sDefaultNum)
        #numboard.doModal()
        if numboard != None:
                return numboard
