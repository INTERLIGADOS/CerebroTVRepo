...
from resources.lib.gui.buiElement  import bGuiElement
...
class bGui:
    ...
    def addNextPage(self, site, function, oParams='', totalPages = 0):
        ...
        '''
        guiElement = bGuiElement('>>>',site,function)
    ...
    def setView(self, content='movies'):
        ...
        if bConfig().getSetting('auto-view')=='true':
            xbmc.executebuiltin("Container.SetViewMode(%s)" % bConfig().getSetting(content+'-view') )
    ...
    def showNofication(self, sTitle, iSeconds=0):
        ...
        xbmc.executebuiltin("Notification(%s,%s,%s,%s)" % (bConfig().getLocalizedString(30308), (bConfig().getLocalizedString(30309) % str(sTitle)), iSeconds, common.Addon.getAddonInfo('icon')))
