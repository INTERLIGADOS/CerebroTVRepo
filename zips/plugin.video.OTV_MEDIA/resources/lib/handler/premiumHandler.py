...
PathCache = xbmc.translatePath(xbmcaddon.Addon('plugin.video.OTV_MEDIA').getAddonInfo("profile"))
...
class cPremiumHandler:
    ...
    def isPremiumModeAvailable(self):
        ...
        if (bIsPremium == 'true'):
            cConfig().log("Uses premium account for hoster " +  str(self.__sHosterIdentifier))
            return True

        cConfig().log("Uses free account to hoster: " + str(self.__sHosterIdentifier))
        return False
    ...
    def Authentificate(self):
        ...
        cGui().showInfo(self.__sDisplayName, 'managed authentication' , 5)
        print 'managed authentication'
