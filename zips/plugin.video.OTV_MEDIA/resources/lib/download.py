...
try:
    ...
    Memorise = StorageServer.StorageServer("OTV_MEDIADownloader")
except:
    print 'Der Download wird nicht korrekt funktionieren'
...
class cDownloadProgressBar(threading.Thread):
    def __init__(self, *args, **kwargs):
        ...
        if (kwargs):
            ...
            if'FastMode' in kwargs:
                print 'Kostenfreier Download von Turbo-Modus'
    ...
    def _StartDownload(self): 
        ...
        #self.Memorise.set("OTV_MEDIADownloaderWorking", "1")
        ...
        cConfig().showInfo('OTV_MEDIA', 'Gebühren- Schritte')
        ...
        while not (self.processIsCanceled or diag.isFinished()):
            ...
            #if self.Memorise.get("OTV_MEDIADownloaderWorking") == "0":
        ...
        Memorise.unlock("OTV_MEDIADownloaderLock")
        ...
        if (TotDown == iTotalSize) and (iTotalSize > 10000):
            ...
            try:
                ...
                cConfig().showInfo('Gebühren- Termine', self.__sTitle)
                print 'Gebühren- Termine : %s' % self.__sTitle
            ...
            try:
                cDb().update_download(meta)
                cConfig().showInfo('Gebühren- verhaftet', self.__sTitle)
                print 'Gebühren- Arrete : %s' % self.__sTitle
                self.RefreshDownloadList()
            except:
                pass
            return
    ...
    def run(self):
        ...
        try:
            ...
            cConfig().showInfo('Fehler initialisation', 'Download error')
            return
        
        if not Memorise.lock("OTV_MEDIADownloaderLock"):
            cConfig().showInfo('Downloads bereits begonnen', 'Download error')
            #self.oUrlHandler.close()
            return
    ...
    def StopAll(self):
        ...
        #self.Memorise.set("OTV_MEDIADownloaderWorking", "0")
...
class cDownload:  
    ...
    def isDownloading(self):
        ...
        if not Memorise.get('OTV_MEDIADownloaderLock'):
            return False
        return True
    ...
    def download(self, sDBUrl, sTitle,sDownloadPath,FastMode = False):
        ...
        if self.isDownloading():
            cConfig().showInfo('Downloads bereits begonnen', 'Fehler')
            return False
        ...
        if (aLink[0] == True):
            ...
            cConfig().showInfo('nicht auflösbar Link', sTitle)
            return False
            
        if (not sUrl.startswith('http')) or sUrl.split('|')[0].endswith('.m3u8') :
            cConfig().showInfo('Format unterstützt nicht', sTitle)
            return False
        ...
        try:
            ...
            cConfig().log("Download ok")
            cConfig().log(sDownloadPath)
            

        except:
            #print_exc()
            cConfig().showInfo('Download unmöglich', sTitle)
            cConfig().log("Download unmöglich")
            return False
    ...
    def getDownload(self):
        ...
        meta = {'title': 'Startliste'}
        item = xbmcgui.ListItem('Start list', iconImage=cConfig().getRootArt()+'download.png')
        ...
        oGui.addDir(SITE_IDENTIFIER, 'StopDownloadList', 'Stop download', 'download.png', oOutputParameterHandler)

        oOutputParameterHandler = cOutputParameterHandler()
        oGui.addDir(SITE_IDENTIFIER, 'getDownloadList', 'Download List', 'download.png', oOutputParameterHandler)
        
        oOutputParameterHandler = cOutputParameterHandler()
        oGui.addDir(SITE_IDENTIFIER, 'CleanDownloadList', 'Reinigen Sie die Liste (fertige Files)', 'download.png', oOutputParameterHandler)
    ...
    def DelFile(self):
        ...
        oDialog = cConfig().createDialogYesNo('Wirklich diese Datei löschen? Der Betrieb nicht reversibel.')
        if (oDialog == 1):
            meta = {}
            meta['url'] = ''
            meta['path'] = path
            ...
            try:
                ...
                xbmcvfs.delete(path)
                cConfig().showInfo('OTV_MEDIA', 'gelöschte Datei')
                cConfig().update()
            except:
                cConfig().showInfo('OTV_MEDIA', 'Fehler, Datei kann nicht gelöscht werden')
    ...
    def StartDownloadList(self):
        cConfig().showInfo('Information', 'Starten Sie die komplette Liste ')
    ...
    def getDownloadList(self):
        ...
        for data in row:
            ...
            if status == '0':
                ...
                sStatus='[COLOR=red][laufend] [/COLOR]'
            elif status == '2':
                sStatus='[COLOR=green][End] [/COLOR]'
        ...
        if (sTitle != False and len(sTitle) > 0):
            ...
            if (sPath != ''):
                ...
                if xbmcvfs.exists(sDownloadPath):
                    cConfig().showInfo('Name bereits verwendet', sTitle)
                    return self.AddDownload(meta)
                else:
                    xbmcvfs.File(sDownloadPath, 'w')

                try:
                    cConfig().log("Zusatz-Download-Liste " + str(sUrl))
                    meta['title'] = sTitle
                    ...
                    #print_exc()
                    cConfig().showInfo('nicht herunterladen', sTitle)
                    cConfig().log("nicht herunterladen")
    ...
    def AddtoDownloadList(self):
        ...
        cConfig().log("Download " + sMediaUrl)
            ...
            if not self.isDownloading():
                ...
                if row:
                    ...
                    if (self.download(url,title,path,True) == True): #Download in fastmode
                        ...
                        cConfig().showInfo('fehlgeschlagen herunterladen', 'Error')
        return
