...
class UnCaptchaReCaptcha:
    ...
    def _collect_api_info(self):
        ...
        html = getUrl("https://www.google.com/recaptcha/api.js?onload=setup&render=explicit")
...
def performCaptcha(sitename,cj,returnpage=True,captcharegex='sitekey:"(.*?)"',lang="fr",headers=None):

    gcookieJar = getg()
   
    sitekey=re.findall(captcharegex,sitename)
    token=""
    if len(sitekey)>=1:
