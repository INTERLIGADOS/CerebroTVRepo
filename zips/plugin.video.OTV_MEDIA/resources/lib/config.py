...
class cConfig():
    ...
    def __init__(self):
        ...
        if (self.__bIsDharma):
            ...
            self.__sFileDB = os.path.join(self.__oCache,'OTV_MEDIA.db')
    ...
    def createDialogOK(self, label):
        ...
        oDialog.ok('OTV_MEDIA', label)  
        return oDialog
        
    def createDialogYesNo(self, label):
        oDialog = xbmcgui.Dialog()
        qst = oDialog.yesno("OTV_MEDIA", label)
        return qst
    ...
    def finishDialog(self, dialog):
        if xbmcgui.Window(10101).getProperty('search') != 'true':
            dialog.close()
            xbmc.log('\t[PLUGIN] OTV_MEDIA: close dialog')
            del dialog
            return False
    ...
    def error(self, e):
        xbmc.executebuiltin("Notification(%s,%s,%s,%s)" % ('OTV_MEDIA', ('Error: '+str(e)), '10000', self.__sIcon))
        xbmc.log('\t[PLUGIN] OTV_MEDIA Error: '+str(e))
        #cConfig().ERROR.append(e)

    def log(self, e):
        xbmc.log('\t[PLUGIN] OTV_MEDIA: '+str(e))
    ...
    def openerror(self):
        ...
        self.win.getControl(1).setLabel("OTV_MEDIA popup Error")
    ...
    def WindowsBoxes(self, sTitle, sFileName, num,year = ''):
        ...
        if (extendedinfo):
            if num == "2":
                self.showInfo('OTV_MEDIA', 'Launching ExtendInfo')
