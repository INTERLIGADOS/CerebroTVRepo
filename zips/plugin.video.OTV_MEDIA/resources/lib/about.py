...
class cAbout:       
    ...
    def getRootPath(self, folder):
        sMath = cConfig().getAddonPath().replace('plugin.video.OTV_MEDIA', '') 
    ...
    def resultGit(self):
        ...
        try: 
            sUrl = 'https://netcologne.dl.sourceforge.net/project/e2-orhantv1/sites.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request();
            result = json.loads(sHtmlContent)
            
            sUrl = 'https://netcologne.dl.sourceforge.net/project/e2-orhantv1/hosts.json'
            oRequestHandler = cRequestHandler(sUrl)
            sHtmlContent = oRequestHandler.request();
            result += json.loads(sHtmlContent)
        except:
    ...
    def checkupdate(self):
            ...
            #dialog = cConfig().showInfo("OTV_MEDIA", "Cherche les mises a jour")
            ...
            if result:
                ...
                if (sDown != 0):
                    ...
                    cConfig().setSetting('service_time', str(datetime.datetime.now()))
                    dialog = cConfig().showInfo("OTV_MEDIA", "Update available")   
                else:
                    #cConfig().showInfo('OTV_MEDIA', 'Fichier a jour')
                    cConfig().setSetting('service_time', str(datetime.datetime.now()))
    ...
    def checkdownload(self):
            ...
            if result: 
                ...
                #self.TextBoxes('OTV_MEDIA mise à Jour', sContent)
                cConfig().setSetting('service_time', str(datetime.datetime.now()))
                cConfig().setSetting('home_update', str('false'))
