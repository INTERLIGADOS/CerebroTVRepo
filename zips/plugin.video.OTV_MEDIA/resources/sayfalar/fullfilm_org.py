...
SITE_IDENTIFIER = 'fullfilm_org'
SITE_NAME = 'FullFilm.org'

 
URL_MAIN = 'http://www.full-film.org/'
...
def fullfilm(): #affiche les genres
    oGui = cGui()

    oInputParameterHandler = cInputParameterHandler()
    sUrl = 'http://www.full-film.org'
...
def Hosters():
    ...
    oRequestHandler = cRequestHandler(sUrl)
    sHtmlContent = oRequestHandler.request();
    sPattern = '<div class="KralFilm_part">(.+?)<span class="keros">'
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
