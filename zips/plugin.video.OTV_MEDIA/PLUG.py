...
g_ignoreSetResolved=['plugin.video.dramasonline','plugin.video.OTV_MEDIA','plugin.video.shahidmbcnet','plugin.video.SportsDevil','plugin.stream.vaughnlive.tv','plugin.video.ZemTV-shani']
...
addon = xbmcaddon.Addon('plugin.video.OTV_MEDIA')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.OTV_MEDIA-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(garaveliTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(garaveliTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(garaveliTV,New source added.,5000,"+icon+")")
        if not url is None:
...
def SportsDevil():
    print "SportsDevil"
    url = "http://repo.run/zip/plugin.video.SportsDevil/plugin.video.SportsDevil-2016.09.06.zip"
...
def f4mTester():
    print "f4mTester"
    url = "http://repo.run/zip/plugin.video.f4mTester/plugin.video.f4mTester-2.6.6.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
...
def ZemTV():
    print "ZemTV-shani"
    url = "http://repo.run/zip/plugin.video.ZemTV-shani/plugin.video.ZemTV-shani-5.9.8.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
...
def LiveStreamPro():
    print "LiveStreamPro"
    url = "http://repo.run/zip/plugin.video.live.streamspro/plugin.video.live.streamspro-2.7.1.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
...
def FIXResolver():
    print "A47Skin"
    url = "http://repo.run/zip/script.module.urlresolver/script.module.urlresolver.zip"
    addonsDir = xbmc.translatePath(os.path.join('special://home', 'addons')).decode("utf-8")
    packageFile = os.path.join(addonsDir, 'packages', 'aeon.zip')
    ...
    xbmc.executebuiltin("UpdateLocalAddons")
    xbmc.executebuiltin("UpdateAddonRepos")
    xbmcgui.Dialog().ok('FIX Resolver', 'Successfully installed!')
...
#    print 'total m3u links',total
    for other,name,url in match:
        
        if 'tvg-logo' in other:
            iconimage = re_me(other,'tvg-logo=[\'"](.*?)[\'"]')
            if iconimage:
        ...
        if 'type' in other:
            ...
            if mode_type == 'yt-dl':
                ...
                url = url.split('&regexs=')
                #print url[0] getSoup(url,data=None)
                regexs = parse_regex(getSoup('',data=url[1]))

                addLink(url[0], name,iconimage,'','','','','',None,regexs,total)
                continue
            elif mode_type == 'ftv':
                url = 'plugin://plugin.video.F.T.V/?name='+urllib.quote(name) +'&url=' +url +'&mode=125&ch_fanart=na'
        elif '.ts' in url:
            StreamType = 'TSDOWNLOADER'
            url = 'plugin://plugin.video.OTV_MEDIA/?url='+urllib.quote_plus(url)+'&streamtype=' + StreamType + '&name='+urllib.quote(name)+'&mode=555&iconImage=' + iconimage
            
        elif '.m3u8' in url:
            StreamType = 'HLSRETRY'
            url = 'plugin://plugin.video.OTV_MEDIA/?url='+urllib.quote_plus(url)+'&streamtype=' + StreamType + '&name='+urllib.quote(name)+'&mode=555&iconImage=' + iconimage
        addLink(url, name,iconimage,'','','','','',None,'',total)
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(garaveliTV,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('garaveliTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('garaveliTV', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from garaveliTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to garaveliTV Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from garaveliTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to garaveliTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
lakaluka = os.path.join(home, 'Thumbs.db')
if os.path.exists(lakaluka)==True:
    SIFIR = open(lakaluka).read()
else: SIFIR = []
if not url is None:
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(garaveliTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(garaveliTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(garaveliTV,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(garaveliTV,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(garaveliTV,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
