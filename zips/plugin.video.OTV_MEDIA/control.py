...
if xbmcaddon.Addon('plugin.video.OTV_MEDIA').getSetting('download_path') == "":
    dst = os.path.join( os.path.join(xbmc.translatePath("special://userdata/addon_data" ), 'plugin.video.OTV_MEDIA'), 'download', 'tmp')
else:
    dst = os.path.join(xbmcaddon.Addon('plugin.video.OTV_MEDIA').getSetting('download_path'), 'tmp')
