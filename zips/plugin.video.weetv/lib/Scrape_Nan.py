...
def scrape_movie(name,year,imdb):
    ...
    if name is not "" and year is not "":
        ...
        link = nanscrapers.scrape_movie_with_dialog(name, year, imdb, timeout=400, sort_function=sort_function)
        if link is False:
            xbmcgui.Dialog().ok("Movie not found", "No Links Found for " + name + " (" + year + ")")
        else:
            if link:
                url = link['url']
                weeresolver.weeresolver(name,url)
...
def scrape_episode(name,show_year,year,season,episode,imdb):
    ...
    if name is not "" and season is not "" and episode is not "":
        def sort_function(item):
            ...
            return quality
        link = nanscrapers.scrape_episode_with_dialog(name, show_year, year, season, episode, imdb, None, timeout=400, sort_function=sort_function)
        if link is False:
