        ...
        if scraper_links != None:
            for link in scraper_links:
                if dp.iscanceled():
                    return
                fin_list.extend('a')
                dp.update(100/int(len(nanscraper_no))*int(len(scraper_list)),"Links found: ""HD-"+str(int(len(hd_list)-1))+ " /MD-"+str(int(len(mid_list)-1))+ " /SD-"+str(int(len(sd_list)-1))+'','Scrapers left to run: '+str(int(len(nanscraper_no))-int(len(scraper_list)))+' of '+str(len(nanscraper_no)))
                if dp.iscanceled():
                    return
                sort_qual(link)
...
def scrape_episode(title,show_year,year,season,episode,imdb):
    ...
    try:
        populator = scrape_episode(title, show_year, year, season, episode,imdb,'', timeout=1000)
    except:
        return
...
def scrape_movie(name,year,imdb):
    ...
    try:
        populator = scrape_movie(name, year, imdb, timeout=1000)
    except:
        return
    return_links(populator)
