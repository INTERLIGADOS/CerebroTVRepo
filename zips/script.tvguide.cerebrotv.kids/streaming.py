...
class StreamsService(object):
    def __init__(self, addon):
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguide.cerebrotv.kids', 'addons.ini'))
            xbmc.log('[script.tvguide.cerebrotv.kids] FTV addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvguide.cerebrotv.kids', customFile.split('/')[-1]))
            xbmc.log('[script.tvguide.cerebrotv.kids] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
