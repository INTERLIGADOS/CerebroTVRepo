...
if __name__ == '__main__':
    ...
    try:
        ADDON = xbmcaddon.Addon('script.tvguide.cerebrotv.kids')
        ...
        if ADDON.getSetting('background.service') == 'true':
            ...
            xbmc.log("[script.tvguide.cerebrotv.kids] Background service started...", xbmc.LOGDEBUG)
            Service()
            ...
            while not monitor.abortRequested():
                ...
                xbmc.log("[script.tvguide.cerebrotv.kids] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                if monitor.waitForAbort(waitTime):
                    # Abort was requested while waiting. We should exit
                    break
                xbmc.log("[script.tvguide.cerebrotv.kids] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                
    except source.SourceNotConfiguredException:
        pass  # ignore
    except Exception, ex:
        xbmc.log('[script.tvguide.cerebrotv.kids] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
