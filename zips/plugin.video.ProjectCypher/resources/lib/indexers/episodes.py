    ...
    def tvdb_list(self, tvshowtitle, year, imdb, tvdb, lang, limit=''):
        ...
        for item in episodes:
            try:
                ...
                title = '[B][COLOR yellow] %s [/COLOR][/B]' % (title.encode('utf-8'))
                ...
                if label == '': label = '0'
                label = client.replaceHTMLCodes(label)
                label = '[B][COLOR yellow] %s [/COLOR][/B]' % (label.encode('utf-8'))
    ...
    def trakt_episodes_list(self, url, user, lang):
        ...
        def items_list(i):
            ...
            try:
                ...
                tvshowtitle = '[B][COLOR yellow] %s [/COLOR][/B]' % (i['tvshowtitle'])
                imdb, tvdb = i['imdb'], i['tvdb']
    ...
    def episodeDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                if i['label'] == '0':
                    label = '[B]%sx%02d[/B] . [B]%s[/B] [B]%s[/B]' % (i['season'], int(i['episode']), 'Episode', i['episode'])
                else:
                    label = '%sx%02d . %s' % (i['season'], int(i['episode']), i['label'])
                if multi == True:
                    label = '[B][COLOR yellow]%s[/COLOR][/B] - [B]%s[/B]' % (i['tvshowtitle'], label)
