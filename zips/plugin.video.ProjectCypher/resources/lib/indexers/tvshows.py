...
class tvshows:
    def __init__(self):
        ...
        self.keywords_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&release_date=,date[0]&keywords=%s&sort=%s,asc&count=%s&start=1' % ( '%s', self_uservar, self_useramount)
        self.persons_link = 'http://www.imdb.com/search/name?count=100&name='
        self.personlist_link = 'http://www.imdb.com/search/name?count=100&gender=male,female'
        self.popular_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&num_votes=100,&release_date=,date[0]&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.airing_link = 'http://www.imdb.com/search/title?title_type=tv_episode&release_date=date[1],date[0]&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.active_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&num_votes=10,&production_status=active&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.premiere_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&languages=en&num_votes=10,&release_date=date[60],date[0]&sort=moviemeter,asc&count=%s&start=1' % (self_useramount)
        self.rating_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&num_votes=5000,&release_date=,date[0]&sort=%s,desc&count=%s&start=1' % ( self_uservar, self_useramount)
        self.views_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&num_votes=100,&release_date=,date[0]&sort=num_votes,desc&count=%s&start=1' % (self_useramount)
        self.person_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&release_date=,date[0]&role=%s&sort=year,desc&count=%s&start=1' % ('%s', self_useramount)
        self.genre_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&release_date=,date[0]&genres=%s&sort=%s,asc&count=%s&start=1' % ( '%s', self_uservar, self_useramount)
        self.keyword_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&release_date=,date[0]&keywords=%s&sort=moviemeter,asc&count=%s&start=1' % ('%s', self_useramount)
        self.language_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&num_votes=100,&production_status=released&primary_language=%s&sort=%s,asc&count=%s&start=1' % ('%s', self_uservar, self_useramount)
        self.certification_link = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&release_date=,date[0]&certificates=us:%s&sort=%s,asc&count=%s&start=1' % ('%s', self_uservar, self_useramount)
    ...
    def genres(self):
        ...
        for i in genres: self.list.append(
            {
                'name': '[B][COLOR yellow] %s [/COLOR][/B]' % (cleangenre.lang(i[0], self.lang)),
    ...
    def networks(self):
        ...
        for i in networks: self.list.append({'name': '[B][COLOR yellow] %s [/COLOR][/B]' % (i[0]), 'url': self.tvmaze_link + i[1], 'image': 'networks.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': '[B][COLOR yellow] %s [/COLOR][/B]' % (str(i)), 'url': self.certification_link % str(i).replace('-', '_').lower(), 'image': 'certificates.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def tvshowDirectory(self, items):
        ...
        for i in items:
            try:
                ...
                item = control.item(label='[B][COLOR yellow] %s [/COLOR][/B]' % (label))
