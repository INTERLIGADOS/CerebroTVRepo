...
class indexer:
	...
	def root(self):
		try:
			regex.clear()
			url = base64.b64decode ('aHR0cDovL2N5cGhlci1tZWRpYS5jb20vdGVzdC54bWw=')
			self.list = self.pycyph_list(url)
	...
	def getx(self, url, worker=False):
		try:
			...
			url = regex.resolve(r)
			self.list = self.pycyph_list('', result=url)
			self.addDirectory(self.list)
			return self.list
		except:
			pass
	...
	def developer(self):
		try:
			...
			self.list = self.pycyph_list('', result=result)
	...
	def addSearch(self, url):
			...
			self.list = [] ; threads = [] 
			for link in links: threads.append(workers.Thread(self.pycyph_list, link))
			...
			for i in self.list:
				try:
					name = ''
					if not i['vip'] in ['pycyph TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
					name += i['name']
					i.update({'name' : name})
				except:
					pass
	...
	def pycyph_list(self, url, result=None):
