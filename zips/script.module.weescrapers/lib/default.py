...
import sys
import xbmcvfs
from weescrapers.common import clean_title
...
def test():
    ...
    if dialog.yesno("weescrapers Testing Mode", 'Clear cache?'):
        weescrapers.clear_cache()
    try:
        dbcon = database.connect(os.path.join(
            xbmc.translatePath(xbmcaddon.Addon("script.module.weescrapers").getAddonInfo('profile')).decode('utf-8'),
            'url_cache.db'))
        dbcur = dbcon.cursor()
    except:
        dialog.ok("weescrapers Testing Mode", 'Error connecting to db')
        sys.exit()

    num_movies = len(movies)
    if num_movies > 0:
        pDialog.create('weescrapers Testing mode active', 'please wait')
        index = 0
        for movie in movies:
            index += 1
            title = movie['title']
            ...
            links_scraper = weescrapers.scrape_movie(title, year, imdb)
    ...
    if num_shows > 0:
        ...
        for show in shows:
            ...
            links_scraper = weescrapers.scrape_episode(title, show_year, year, season, episode, imdb, tvdb)
            links_scraper = links_scraper()
            for scraper_links in links_scraper:
                if pDialog.iscanceled():
                    break
    ...
    dialog.textviewer("weescrapers Testing Mode", resultstring)


def profile_scrapers(profile_type):
    global movies, shows
    from weescrapers.hl import HostedLink
    import random
