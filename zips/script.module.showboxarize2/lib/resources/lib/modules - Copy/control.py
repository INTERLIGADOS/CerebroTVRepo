...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.showboxarize2.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.showboxarize2.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.showboxarize2.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.showboxarize2.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.showboxarize2.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.showboxarize2.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
