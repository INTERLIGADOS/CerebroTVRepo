...
class indexer:
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.kratos_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.kratos_list('', result=result)
    ...
    def addSearch(self, url=None):
        try:
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.kratos_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['Kratos TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def kratos_list(self, url, result=None):
        try:
...
class resolver:
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            from resources.lib.zmodules import sources
        ...
        try:
            ...
            from resources.lib.zmodules import filmon
