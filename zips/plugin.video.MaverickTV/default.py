...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.MaverickTV Lists-%s]: %s" %(addon_version, string))
...
def SKindex():
    addon_log("SKindex")
    Menu('[COLOR lime][B]Maverick Online Movies[/B][/COLOR]','',999997,'http://mavericktv.net/data/icons/online.jpg','https://i.imgur.com/SQrVkhu.jpg','','','','')#
...
def Mpop(url):
    ...
    for name , year in match:
        ...
        PLAY(name+'  [COLORred]'+year+'[/COLOR]',years,999998,'','',year,name)

def Mpopgen(url):
    html = requests.get(url).content
    match = re.compile('<tr class=".+?".+?<h3>.+?href="(.+?)">(.+?)<.+?<p>(.+?)</p>',re.DOTALL).findall(html)
    for a,b,rest in match:
        Menu('[COLORlime]'+b.upper()+'[/COLOR]',a,999994,'','','','','','')
...
def Mpopgenres(url):
    ...
    for z ,x in match:
        ...
        years = x[1:5]
        PLAY(z+'  [COLORred]'+x+'[/COLOR]',years,999998,'','',x,z)
...
def catchuptv(url):
    ...
    for cont in conts:
        ...
        for link in links:
            ...
            for br in match:
                addDir('[COLORgold]'+alt+'[/COLOR][COLORwhite] - [COLORsteelblue]'+br+'[/COLOR]',href,302,'','','','','','','')
                
def tv_show():
    
    addDir('[COLORlime][B]Tv Shows List[/COLOR][/B]','http://www.tvmaze.com/shows',402,icon,FANART,'','','','','')
    addDir('[COLORsteellime]SEARCH Tv Shows[/COLOR]','',405,icon,FANART,'','','','','')
     

def show_main(url):
    Menu('[COLORsteelblue]SEARCH[/COLOR]','',405,'',FANART,'','')
    html = requests.get(url).content
