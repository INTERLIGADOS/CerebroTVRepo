...
class tvshows:
	def __init__(self):
		...
		self.search_link = control.apollo_static_link+'search.php?type=tv&search='
		...
		self.popular_link = control.apollo_static_link+'list/tv/popular.json'
		self.airing_link = control.apollo_static_link+'list/tv/recent.json'
		self.active_link = control.apollo_static_link+'list/tv/active.json'
		self.premiere_link = control.apollo_static_link+'list/tv/recent.json'
		self.rating_link = control.apollo_static_link+'list/tv/rating.json'
		self.views_link = control.apollo_static_link+'list/tv/views.json'
		...
		self.genre_link = control.apollo_static_link+'list/tv/genre/%s.json'
		self.certification_link = control.apollo_static_link+'list/tv/certification.%s.json'
		self.trending_link = control.apollo_static_link+'list/tv/trending.json'
		...
		try:
			...
			for data in json_object['result']:
				...
				try:
					...
					self.list.append({'title': str(data['title']),'originaltitle': str(data['title']), 'year': str(data['year']), 'code': str(data['imdb']), 'imdb': str(data['imdb']), 'tmdb': '0', 'tvdb': str(data['tvdb']), 'poster': str(data['poster']), 'banner': str(data['banner']), 'fanart': str(data['fanart']), 'premiered': str(data['released']), 'studio': '0', 'genre': str(data['genres']), 'duration': '0', 'rating': str(data['rating']), 'votes': str(data['votes']), 'mpaa': '0', 'cast': '0', 'plot': str(data['plot'])})
				except: 
		...
		try:
			...
			if 'fanart' in self.list[i] and self.list[i]['fanart'] != '0':
				if self.lang == 'en': 
					...
					item = {'title': title, 'year': year, 'imdb': imdb, 'tvdb': tvdb,'plot': plot}
