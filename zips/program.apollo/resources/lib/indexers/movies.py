...
class movies:
	def __init__(self):
		...
		self.search_link = control.apollo_static_link+'search.php?type=movie&search='
		...
		self.popular_link = control.apollo_static_link+'list/movie/popular.json'
		self.views_link = control.apollo_static_link+'list/movie/views.json'
		self.all_link = control.apollo_static_link+'list/movie/all.json'
		self.featured_link = control.apollo_static_link+'list/movie/recent.json'
		...
		self.genre_link = control.apollo_static_link+'list/movie/genre/%s.json'
		...
		self.certification_link = control.apollo_static_link+'list/movie/certification.%s.json'
		self.year_link = control.apollo_static_link+'list/movie/year/%s.json'
		self.boxoffice_link = control.apollo_static_link+'list/movie/box.json'
		self.oscars_link = control.apollo_static_link+'list/movie/oscars.json'
		self.theaters_link = control.apollo_static_link+'list/movie/theaters.json'
		self.trending_link = control.apollo_static_link+'list/movie/trending.json'
	...
	def get(self, url, idx=True):
		try:
			...
			elif u in control.apollo_link or u in control.apollo_static_link:
				self.list = cache.get(self.apollo_list, 24, url)
				if idx == True: self.worker();
	...
	def years(self):
		...
		for i in range(int(year)-0, int(year)-50, -1): self.list.append({'name': str(i), 'url': self.year_link % (str(i)), 'image': 'years.png', 'action': 'movies'})
		self.addDirectory(self.list)
		return self.list
	...
	def worker(self, level=1):
		...
		response = get(control.apollo_static_link+'list/movie/all.json')
		try:
			json_object = json.loads(response.text)
		except:
			print "### ERROR Load JSON: {0}".format(json.decoder.linecol(doc, pos))
			pass

		for i in range(0, total): 
			if self.list[i]['imdb'] not in json_object["result"]:
				self.list[i]['imdb'] = '0'
	...
	def apollo_list(self, url):
		try:
			...
			for data in json_object['result']:
				poster = '0'
				try: poster = str(data['poster'])
				except: pass
				banner = '0'
				try: banner = str(data['banner'])
				except: pass
				fanart = '0'
				try: fanart = str(data['fanart'])
				except: pass
				runtime = '0'
				try: runtime = str(data['runtime'])
				except: pass
				released = '0'
				try: released = datetime.datetime.fromtimestamp(int(data['released'])).strftime('%Y-%m-%d')
				except: pass

				try: 
					#self.list.append({'title': str(data['title']),'originaltitle': str(data['title']),'year': str(data['year']),'genre': str(data['genre']),'duration': str(data['duration']),'rating': str(data['rating']), 'votes': str(data['votes']),'mpaa': str(data['mpaa']), 'director': str(data['director']), 'writer': str(data['writer']),'cast': '0', 'plot': str(data['plot']),'imdb': str(data['imdb']),'poster': poster,'tvdb': '0',next:'0'})
					self.list.append({'imdb': str(data['imdb']),'title': str(data['title']), 'year': str(data['year']), 'code': str(data['imdb']), 'tmdb': str(data['tmdb']), 'poster': poster, 'banner': banner, 'fanart': fanart, 'premiered': released, 'studio': '', 'genre': str(data['genres']),'cast': '0', 'duration': runtime, 'rating': str(data['rating']), 'votes': str(data['votes']), 'mpaa': '', 'director': str(data['director']), 'writer': str(data['writer']), 'plot': str(data['plot'])})
					#self.list.append({'title': title, 'originaltitle': title, 'year': year, 'genre': genre, 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': director, 'cast': cast, 'plot': plot, 'imdb': imdb, 'tvdb': '0', 'poster': poster, 'next': next})
				except: 
					print "####### ERROR JSON Rec: {0}".format(data)
					pass
			return self.list
		except:
			return []
	...
	def super_info(self, i):
		try:
			...
			if 'fanart' in self.list[i] and self.list[i]['fanart'] != '0':
				if self.lang == 'en': 
					return
				else:
					try:
					...
					item = {'title': title, 'year': self.list[i]['year'], 'imdb': self.list[i]['imdb'],'plot': plot}
	...
	def movieDirectory(self, items):
		...
		for i in items:
			try:
				...
				url = '%s?action=apollo&title=%s&year=%s&imdb=%s&meta=%s&t=%s' % (sysaddon, systitle, year, imdb, sysmeta, self.systime)
