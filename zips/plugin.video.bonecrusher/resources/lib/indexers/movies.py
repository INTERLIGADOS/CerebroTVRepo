...
class movies:
    ...
    def genres(self):
        try:
            ...
            for i in range(0, len(self.list)): self.list[i].update({'image': 'icon.png', 'action': 'movies'})
            self.addDirectory(self.list)
            return self.list
        except:
            return
    ...
    def tmdb_custom_list(self, url):
        # print ("Bone Crusher LISTS", url)
        try:
            result = client.request(url)
            result = json.loads(result)
            items = result['items']
            # print ("Bone Crusher LISTS", items)
        except:
            return
        ...
        for item in items:
            try:
                ...
                title = client.replaceHTMLCodes(title)
                title = title.encode('utf-8')
                # print ("Bone Crusher LISTS", title)

                year = item['release_date']
                year = re.compile('(\d{4})').findall(year)[-1]
                year = year.encode('utf-8')
                # print ("Bone Crusher LISTS", year)
                tmdb = item['id']
                tmdb = re.sub('[^0-9]', '', str(tmdb))
                tmdb = tmdb.encode('utf-8')
                # print ("Bone Crusher LISTS", tmdb)

                poster = item['poster_path']
                if poster == '' or poster == None: raise Exception()
                else: poster = '%s%s' % (self.tmdb_poster, poster)
                poster = poster.encode('utf-8')
                # print ("Bone Crusher LISTS", poster)

                fanart = item['backdrop_path']
                if fanart == '' or fanart == None: fanart = '0'
                if not fanart == '0': fanart = '%s%s' % (self.tmdb_image, fanart)
                fanart = fanart.encode('utf-8')
                # print ("Bone Crusher LISTS", fanart)

                premiered = item['release_date']
                try: premiered = re.compile('(\d{4}-\d{2}-\d{2})').findall(premiered)[0]
                except: premiered = '0'
                premiered = premiered.encode('utf-8')
                # print ("Bone Crusher LISTS", premiered)

                rating = str(item['vote_average'])
                if rating == '' or rating == None: rating = '0'
                rating = rating.encode('utf-8')
                # print ("Bone Crusher LISTS", rating)
                ...
                if votes == '' or votes == None: votes = '0'
                votes = votes.encode('utf-8')
                # print ("Bone Crusher LISTS", votes)

                plot = item['overview']
                if plot == '' or plot == None: plot = '0'
                plot = client.replaceHTMLCodes(plot)
                plot = plot.encode('utf-8')
                # print ("Bone Crusher LISTS", plot)

                tagline = re.compile('[.!?][\s]{1,2}(?=[A-Z])').split(plot)[0]
                try: tagline = tagline.encode('utf-8')
                except: pass
                # print ("Bone Crusher LISTS", tagline)
