...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.wargames.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.wargames.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.wargames.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.wargames.artwork)')
...
def moderator():
    netloc = [urlparse.urlparse(sys.argv[0]).netloc, '', 'plugin.video.live.streamspro', 'plugin.video.eclipse', 'plugin.video.cpstreams', 'plugin.video.streamarmy', 'plugin.video.tinklepad', 'plugin.video.metallic']

    if not infoLabel('Container.PluginName') in netloc: sys.exit()


def metaFile():
    if condVisibility('System.HasAddon(script.wargames.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.wargames.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
